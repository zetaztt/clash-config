...
打开终端显示的本地地址（默认是 `http://localhost:4200`）。主页仅展示已配置 Provider 的名称、订阅主机名和住宅节点的基本信息；订阅令牌、用户名和密码不会显示在主页。
订阅和每个住宅节点各有独立的“编辑”弹窗；点击“添加订阅”或“添加节点”可继续添加配置。在相应弹窗中填写：

- 零份或多份代理订阅：每份使用唯一的 Provider 名称，以及 HTTP 或 HTTPS 订阅地址；
- 零个或多个住宅节点：节点名称、服务器、端口、用户名和密码。

填写订阅地址后，可以点击“读取名称”。网页会直接向该订阅地址发送一次普通 GET 请求，优先读取
`Content-Disposition` 的 `filename*`，没有时再读取 `filename`，并把结果填入可编辑的 Provider
名称。此请求不会经过本站服务器，但会把订阅地址正常发送给订阅服务本身。

浏览器不能覆盖 `User-Agent` 来伪装 Clash Verge Rev；跨域订阅服务还必须允许网页访问，并通过
CORS 暴露 `Content-Disposition` 响应头。因此部分订阅无法自动读取名称，此时应按 Clash Verge
Rev 中显示的名称手动填写。网页不会使用公共中转服务转发包含令牌的订阅请求。

代理 Provider 可以为空。没有 Provider 时“代理节点”组仍会保留，并生成空的 `use: []`；不会
自动加入 `REJECT` 或 `DIRECT`。依赖该组的连接没有可选上游节点，直至添加至少一份 Provider。
...
页面会把 Provider 名称、订阅地址和住宅节点内容自动保存到当前站点的浏览器 Local Storage，并在下次打开时恢复。旧版保存的单份 `Airport URL` 会自动迁移为名为 `airport` 的 Provider。
点击“清除表单和已保存数据”可同时清空页面与持久化副本。“复制配置”会把完整 YAML 写入系统剪切板；点击“生成并下载 YAML”后，浏览器会下载
`clash-config.yaml`。保存与生成过程完全在浏览器内完成；只有用户主动点击“读取名称”时，网页才会直接请求对应的订阅服务。
...
住宅节点可以为空；存在住宅节点时，节点名不能与页面提示的代理组或 Mihomo 内置策略名称冲突，
并且每个节点名必须唯一。

所有订阅 Provider 都会加入 `代理节点`。所有住宅节点都会加入 `住宅节点`，并通过
`代理节点` 建立链式代理。一般业务代理规则使用 `代理策略` 组，可在 `代理节点` 和
`住宅节点` 之间选择；ChatGPT、Claude 和 Cloudflare 验证等现有住宅风控规则使用
`风控策略`，默认选择 `住宅节点`，也可切换为 `代理策略`。最终兜底规则使用 `最终策略`
组，可在 `代理策略`、`风控策略` 和 `DIRECT`（直连）之间选择。
