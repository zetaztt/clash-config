...
每份订阅和每个住宅节点各有独立的“编辑”弹窗；点击“添加订阅”或“添加节点”会直接打开新条目的编辑弹窗。在相应弹窗中填写：

- 零份或多份代理订阅：每份使用唯一的 Provider 名称，以及 HTTP 或 HTTPS 订阅地址；
- 零个或多个住宅节点：节点名称、服务器、端口、用户名和密码。

新增条目在弹窗中只是临时草稿；通过完整校验并点击“完成”后才会加入主页列表并自动保存。关闭弹窗或点击“取消添加”会丢弃草稿。

Provider 名称由用户手动填写；网页不会为了识别名称而请求订阅地址。
...
`clash-config.yaml`。保存与生成过程完全在浏览器内完成，不会把表单内容发送给服务器。
