...
住宅节点名不能与 `代理节点`、`住宅节点`、`广告拦截`、`最终代理`、`DIRECT`，以及
`REJECT`、`REJECT-DROP`、`PASS`、`PASS-RULE`、`COMPATIBLE`、`GLOBAL` 等 Mihomo
内置名称冲突。

所有住宅节点都会加入 `住宅节点`，并通过 `代理节点` 建立链式代理。最终兜底规则使用
`最终代理` 组，可在 `代理节点`、`住宅节点` 和 `DIRECT`（直连）之间选择。
...
其他域名使用 Cloudflare 和 Google DoH，并明确通过 `代理节点` 连接，避免海外 DNS 请求被
...
`apple` 分类流量使用 `代理节点`；`apple-cn` 规则必须位于更宽泛的 `apple` 规则之前。
...
外层规则块；没有优先级差异的同策略规则直接聚合在一个块中。

所有内置规则都作为专项规则直接维护。当前优先级以 `rulesConfigs` 的声明顺序为准，可以按
实际路由需求调整；生成器只负责保持声明顺序，并在末尾追加 `MATCH,最终代理`。
...
无法等价表达的独立 `port`、`network` 或 `protocol` 条件。中国和海外公共 DNS IP 规则使用
`no-resolve`；局域网、海外服务和中国 IP 规则允许解析。不同规则块之间的重复
